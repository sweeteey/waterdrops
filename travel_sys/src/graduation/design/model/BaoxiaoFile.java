package graduation.design.model;

public class BaoxiaoFile {
	private int id;

	public BaoxiaoFile() {
	}

	public void setId(int id) {
		this.id = id;
	}

	public int getId() {
		return this.id;
	}

	private String s_0;
	public void setS_0(String s_0) {
		this.s_0 = s_0;
	}

	public String getS_0() {
		return this.s_0;
	}

	private String s_1;
	public void setS_1(String s_1) {
		this.s_1 = s_1;
	}

	public String getS_1() {
		return this.s_1;
	}

	private String s_2;
	public void setS_2(String s_2) {
		this.s_2 = s_2;
	}

	public String getS_2() {
		return this.s_2;
	}

	private String s_3;
	public void setS_3(String s_3) {
		this.s_3 = s_3;
	}

	public String getS_3() {
		return this.s_3;
	}

	private String s_4;
	public void setS_4(String s_4) {
		this.s_4 = s_4;
	}

	public String getS_4() {
		return this.s_4;
	}

	private String s_5;
	public void setS_5(String s_5) {
		this.s_5 = s_5;
	}

	public String getS_5() {
		return this.s_5;
	}

	private BaoxiaoStatus baoxiaoStatus;
	public void setBaoxiaoStatus(BaoxiaoStatus baoxiaoStatus) {
		this.baoxiaoStatus = baoxiaoStatus;
	}

	public BaoxiaoStatus getBaoxiaoStatus() {
		return this.baoxiaoStatus;
	}

	private BaoxiaoType baoxiaoType;
	public void setBaoxiaoType(BaoxiaoType baoxiaoType) {
		this.baoxiaoType = baoxiaoType;
	}

	public BaoxiaoType getBaoxiaoType() {
		return this.baoxiaoType;
	}

	private Xibie xibie;
	public void setXibie(Xibie xibie) {
		this.xibie = xibie;
	}

	public Xibie getXibie() {
		return this.xibie;
	}

	private User user;
	public void setUser(User user) {
		this.user = user;
	}

	public User getUser() {
		return this.user;
	}

}
