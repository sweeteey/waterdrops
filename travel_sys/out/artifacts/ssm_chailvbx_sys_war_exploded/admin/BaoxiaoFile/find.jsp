	...
	<script type="text/javascript">
		function topPage() {
			...
			window.location = contextPath
					+ '/BaoxiaoFile/getAllUtil.do?pageModel.currentPageNo=' + pageNo
...
<div>
	<nav class="breadcrumb"> <i class="Hui-iconfont">&#xe67f;</i> 首页
		<span class="c-gray en">&gt;</span>报销审批 <span class="c-gray en">&gt;</span>
		报销审批列表 <a class="btn btn-success radius r"
	...
	<div class="page-container">
		<div class="text-c">
			<form
					action="${pageContext.request.contextPath}/BaoxiaoFile/getAllUtil.do">
				<select id="field" name="field" class="select"
						style="width:120px;height:32px;">
					<option <c:if test="${field == 't_baoxiaoStatus.s_0'}">selected</c:if> value="t_baoxiaoStatus.s_0" />
					审批状态
					</option>
					<option <c:if test="${field == 't_baoxiaoType.s_0'}">selected</c:if> value="t_baoxiaoType.s_0" />
					报销类型
					</option>
					<option <c:if test="${field == 't_xibie.s_0'}">selected</c:if> value="t_xibie.s_0" />
					部门信息
					</option>
		...
		<div class="cl pd-5 bg-1 bk-gray mt-20">
			...
			<c:if test="${user_type == 'admin'}"><a href="javascript:;" onclick="datadel()"
 			...
 			<a class="btn btn-primary radius" style="margin-left:20px;"
			   href="${pageContext.request.contextPath}/BaoxiaoFile/initUtil.do"><i
		...
		<div class="mt-20">
			<form
					action="${pageContext.request.contextPath}/BaoxiaoFile/deleteManyUtil.do"
					...
					<c:forEach var="tmp" items="${pageModel.list}">
						<tr class="text-c">
							...
							<c:if test="${user_type == 'admin'}">
								<td class="td-manage">
										<%--<c:if test="${tmp.s_1 != 'None'}">
											<a style="text-decoration:none;margin-left:15px;" class="ml-5"
											   href="${pageContext.request.contextPath}/admin/download.jsp?filename=${tmp.s_0}" title="下载">
												<i class="Hui-iconfont">&#xe6df;</i>下载 </a>
										</c:if>--%>
									<a style="text-decoration:none;margin-left:15px;"
									   class="ml-5"
									   href="${pageContext.request.contextPath}/BaoxiaoFile/selectUtil.do?id=${tmp.id }"
									   title="编辑"><i class="Hui-iconfont">&#xe6df;</i> 编辑</a> <a
										style="text-decoration:none;margin-left:15px;" class="ml-5"
										href="${pageContext.request.contextPath}/BaoxiaoFile/deleteUtil.do?id=${tmp.id }"
							...
							<c:if test="${user_type != 'admin'}">
								<td class="td-manage">
										...
										title="下载"><i class="Hui-iconfont">&#xe6df;</i>下载 </a>  --%>
									<a style="text-decoration:none;margin-left:15px;"
									   class="ml-5"
									   href="${pageContext.request.contextPath}/BaoxiaoFile/selectUtil.do?id=${tmp.id }"
